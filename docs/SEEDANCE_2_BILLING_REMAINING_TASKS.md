...
| 前端生产构建排查 | 已完成，持续优化 | 已给 Vite 构建增加 Node heap 上限、将 code-inspector 限制在 dev server，并对 Billing、TopUp、SelfBilling、Playground、Setting 等控制台路由做 lazy loading；`npm run build` 已通过，最大业务 chunk 从约 8.22 MB 降至约 6.15 MB。构建仍提示部分 vendor、Markdown、Mermaid chunk 偏大。 | 后续可继续拆分 MarkdownRenderer、Mermaid/图表库、Semi UI 重页面。 |

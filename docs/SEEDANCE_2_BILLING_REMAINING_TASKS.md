...
5. 增加用户自助账单只读 API `/api/billing/self/summary`、`/api/billing/self/export`、`/api/billing/self/statements` 和独立用户账单页 `/console/my-billing`，强制按当前登录用户过滤，避免越权看账。
6. 增加任务计费审计字段，包括资金来源、订阅 ID、预扣额度和实际额度。
7. 增加 Seedance 2.0 图片、视频和 callback URL 的基础 SSRF 校验。
8. 增加 Seedance 2.0 验收手册、真实接口 smoke test 脚本、服务端 readiness API、验收证据归档脚本和证据完整性校验脚本。
...
| 前端生产构建排查 | 已完成，持续优化 | 已给 Vite 构建增加 Node heap 上限、将 code-inspector 限制在 dev server，并对 Billing、TopUp、SelfBilling、Playground、Setting 等控制台路由做 lazy loading；MarkdownRenderer 已将 Mermaid 改为按需动态加载；Dashboard 已将 VChart 组件和 Semi 主题初始化改为动态加载；i18n 语言资源改为启动按当前语言加载、切换语言时动态加载。`npm run build` 已通过，最大业务 chunk 从约 8.22 MB 降至约 5.05 MB。构建仍提示部分 vendor、Markdown、Semi UI chunk 偏大。 | 后续可继续拆分 MarkdownRenderer 其它高亮/数学依赖、Semi UI 重页面和全局 vendor/index 包。 |
...
| 任务 | 状态 | 说明 | 建议处理 |
| --- | --- | --- | --- |
| 月度账单报表表 | 基础完成 | 已新增 `billing_statements` 快照表、生成/查询接口、后台账单页快照面板、用户自助月结快照 API，以及默认关闭的自动上月账单快照任务。 | 生产开启前需配置 `BILLING_STATEMENT_AUTO_*` 并在预发确认账期口径；PDF/邮件发送仍待后续产品化。 |
...
| 资金来源与任务维度对账 | 已完成 | 汇总 API、后台账单页、用户自助账单 API 和 CSV 导出已支持 `billing_source` 与 `task_id`，可按钱包余额、订阅额度和单笔任务聚合、筛选和对账。 | 预发用真实钱包/订阅任务各跑一笔，并用返回的 `task_id` 保存页面和 CSV 对账截图。 |
| 客户可见账单页 | 已完成 | 钱包/充值页已新增“我的 Seedance 账单”面板，普通用户也可以通过独立页面 `/console/my-billing` 查看自己的汇总、CSV 和月结快照；后端会强制绑定当前登录用户。 | 预发保存 `/console/my-billing` 页面截图和自助 CSV。后续如需更完整财务体验，可增加图表、PDF/邮件发送和更多财务字段。 |

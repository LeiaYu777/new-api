...
7. 增加 Seedance 2.0 验收手册、真实接口 smoke test 脚本、验收证据归档脚本和证据完整性校验脚本。
...
| 生产监控与告警 | 基础完成 | 已新增后台账单告警接口、页面面板、Prometheus 文本导出接口 `/api/billing/metrics`、预检脚本指标抓取校验、Prometheus 抓取样例、告警规则、Grafana Dashboard JSON 和 Grafana 接入手册，覆盖退款率、任务失败率、超时任务、待处理积压、worker 滞后、余额不足和上游错误信号。 | 预发执行 `CHECK_BILLING_METRICS=true ADMIN_ACCESS_TOKEN=... ADMIN_USER_ID=... scripts/seedance-billing-preflight.sh`，并按 `docs/SEEDANCE_2_MONITORING.md` 接入 Prometheus/Grafana 或企业告警系统。 |
...
9. Prometheus 指标：`GET /api/billing/metrics?model_name=doubao-seedance-2-0%`
10. 监控接入手册：`docs/SEEDANCE_2_MONITORING.md`
11. Prometheus 抓取样例：`deploy/observability/seedance-billing-prometheus.yml`
12. Prometheus 告警规则：`deploy/observability/seedance-billing-alert-rules.yml`
13. Grafana Dashboard：`deploy/observability/seedance-billing-grafana-dashboard.json`

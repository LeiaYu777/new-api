...
| 前端生产构建排查 | 已完成，持续优化 | 已给 Vite 构建增加 Node heap 上限、将 code-inspector 限制在 dev server，并对 Billing、TopUp、SelfBilling、Playground、Setting、认证页等路由做 lazy loading；MarkdownRenderer 已将 Mermaid 改为按需动态加载；Dashboard 已将 VChart 组件和 Semi 主题初始化改为动态加载；i18n 语言资源改为启动按当前语言加载、切换语言时动态加载。`npm run build` 已通过，最大业务 chunk 从约 8.22 MB 降至约 4.96 MB。构建仍提示部分 vendor、Markdown、Semi UI chunk 偏大。 | 后续可继续拆分 MarkdownRenderer 其它高亮/数学依赖、Semi UI 重页面和全局 vendor/index 包。 |

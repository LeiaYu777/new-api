...
5. 增加用户自助账单只读 API `/api/billing/self/summary`、`/api/billing/self/export`、`/api/billing/self/statements`，强制按当前登录用户过滤，避免越权看账。
...
| 客户可见账单页 | 基础完成 | 钱包/充值页已新增“我的 Seedance 账单”面板，普通用户可以通过 `/api/billing/self/*` 查看自己的汇总、CSV 和月结快照，后端会强制绑定当前登录用户。 | 后续可按客户 UI 口径继续拆成独立“我的账单”页面，增加图表、PDF/邮件发送和更多财务字段。 |

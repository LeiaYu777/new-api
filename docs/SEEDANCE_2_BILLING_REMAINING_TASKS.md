...
6. 增加 Seedance 2.0 验收手册和真实接口 smoke test 脚本。
...
| 任务 | 状态 | 说明 | 建议处理 |
| --- | --- | --- | --- |
| 远程图片和视频 URI 安全策略 | 未完成 | 代码已限制图片数量和参数取值，但如果允许客户传外部 URL，还需要防 SSRF、防内网探测、防越权对象访问。 | 增加 URL allowlist、禁止内网 IP、使用对象存储中转或安全代理拉取。 |
